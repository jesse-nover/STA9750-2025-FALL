...
```
While we are not specifically focused on blood pressure, there are a number of different health-related statistics provided by the PLACES data, and further investigation may be warranted to consider the complete set of relevant measurements. Some of the other data not being used includes heart disease, physical inactivity, smoking, and binge drinking. 

The focus will be on obesity and diabetes. Plotting the distribution of values will give a sense for what is typical.
```{r}
...
avg_tract_pop <- health_wide |> summarize(avg_pop = mean(totalpopulation))
...
```
<br>
Census tracts can vary greatly in terms of population and incidence. The average census tract has a population of **`r format(round(avg_tract_pop$avg_pop, digits=0), big.mark =',')`**. Note that all of the tracts in this list are below average, and some are quite low. In some cases, a census tract may distort the view of incidence. For example, the CA tract topping this list is entirely populated by residents in an assisted living facility in San Francisco, and the NY tract near the top is only home to residents of a VA assisted living facility in Queens. These tracts are clearly outliers and not representative of the larger population trends.
```{r}
...
```
<br>
Similarly, the NY census tracts appearing on this list for obesity are either entirely or partially populated by inmates in a correctional facility, which also likely does not represent the overall trend well. To get a better read on the trend, it may be more helpful to look at things at the county level.
...
avg_county_pop <- county_summary_diabetes |> summarize(avg_pop = mean(population))
...
county_summary_diabetes |>
  ...
  formatRound(c('Population'), digits = 0, mark=',')
```
<br>
Here we see that the counties with the highest diabetes prevalence are in Alabama and Texas. The average county population is **`r format(round(avg_county_pop$avg_pop, digits=0), big.mark =',')`**. All of the counties appearing on this list are well below the average in population. This may be an indicator that rural communities are more likely to have a high diabetes rate.
...
```
<br>
There are a lot of counties in Mississippi and Alabama topping the list for obesity. These counties are also lower in population than average. It is striking that in most of these counties, more than 50% of the population is obese. 
...
```
As should be clear through the maps, diabetes prevalence is growing substantially. Looking at the data for **`r format(nrow(trend_analysis), big.mark =',')`** counties, the median absolute change is **`r round(median(trend_analysis$absolute_change, na.rm = TRUE), 2)` percentage points** and the median percent change is **`r round(median(trend_analysis$percent_change, na.rm = TRUE), 2)`%.** It is not an exaggeration to characterize the situation as an epidemic.
...
## State Maps
Let's take a closer look at some of the states where the incidence was especially high: Texas, Alabama, Mississippi, and West Virginia.
```{r}
...
# TX stats
tx_health <- health_wide |>
  filter(stateabbr=='TX') |>
  summarize(
    state_pop = sum(totalpopulation),
...
```
Texas has a population of **`r format(tx_health$state_pop, big.mark = ',')`**, an average obesity rate of **`r round(tx_health$avg_obesity, 1)`%** and an average diabetes rate of **`r round(tx_health$avg_diabetes, 1)`%** per census tract. While Texas did not have the very highest levels by state, it is the largest state in terms of population with levels above the median in both measures. This makes it worth a closer look. We can see that levels are especially high in the west and along the border with Mexico.
...
combined_plot2
```
Alabama has a population of **`r format(al_health$state_pop, big.mark = ',')`**, an average obesity rate of **`r round(al_health$avg_obesity, 1)`%** and an average diabetes rate of **`r round(al_health$avg_diabetes, 1)`%** per census tract. Alabama has the third-highest level by states in diabetes and the fourth-highest in obesity. We can see that the highest levels are concentrated in the south and especially southwestern parts of the state.
```{r}
...
combined_plot3
```
Mississippi has a population of **`r format(ms_health$state_pop, big.mark = ',')`**, an average obesity rate of **`r round(ms_health$avg_obesity, 1)`%** and an average diabetes rate of **`r round(ms_health$avg_diabetes, 1)`%** per census tract. Mississippi has the highest level of obesity and the second highest level of diabetes. We notice high levels throughout the state and the highest concentrations are along the western side of the state.
...
combined_plot4
```
West Virginia has a population of **`r format(wv_health$state_pop, big.mark = ',')`**, an average obesity rate of **`r round(wv_health$avg_obesity, 1)`%** and an average diabetes rate of **`r round(wv_health$avg_diabetes, 1)`%** per census tract. West Virginia has the highest level of diabetes and the second highest level of obesity. High levels of each are distributed throughout the state.

## New York City
This project was originally conceived of as a New York City-focused investigation before our team expanded our focus to the whole country. Up to this point in the report, the data has lead us to notice rural extremes. Let's take a moment to look at New York City.

```{r}
#| code-summary: "CODE: Diabetes in NYC"
#| message: false
#| warning: false
# Create NYC subset
nyc_data <- tracts_health %>%
  filter(STATEFP == "36" & COUNTYFP %in% c("005", "047", "061", "081", "085"))
...
# bronx example
bronx_details <- nyc_summary |>
  filter(borough == 'Bronx') |>
  mutate(
    obesity_pop = avg_obesity/100 * population,
    diabetes_pop = avg_diabetes/100 * population
  )

nyc_summary |> format_titles() |>
  datatable(
    options = list(
      searching = FALSE,
      paging = FALSE,
  ...
  formatRound(c('Avg Diabetes', 'Avg Obesity'), digits = 1) |>
  formatRound(c('Population'), digits = 0, mark=',')
```
<br>
While the 5 boroughs—technically counties—did not appear among the top 10 for obesity or diabetes, it should be clear that even a modest percent represents a significant population. For example, the Bronx has a population of **`r format(bronx_details$population, big.mark = ',')`** which means **`r format(bronx_details$obesity_pop, big.mark = ',')`** are obese and **`r format(round(bronx_details$diabetes_pop, 0), big.mark = ',')`** have diabetes.
```{r}
#| code-summary: "CODE: Diabetes in NYC"
#| message: false
#| warning: false
nyc_tracts <- tracts_health %>%
  filter(STATEFP == "36" & COUNTYFP %in% c("005", "047", "061", "081", "085"))
# NYC Diabetes Map
ggplot(nyc_tracts) +
  ...
  labs(
    title = "Diabetes Prevalence in New York City",
    subtitle = "by Census Tract",
    caption = "Data source: CDC PLACES 2025"
  )


```
Here we can see that the Bronx is in fact showing high levels of diabetes along with southeastern Queens and eastern Brooklyn.
```{r}
#| code-summary: "CODE: Obesity in NYC"
#| message: false
#| warning: false

# NYC Obesity Map
ggplot(nyc_tracts) +
  geom_sf(aes(fill = obesity), color = "white", size = 0.1) +
  scale_fill_gradientn(
    colors = c("grey90","#9badff", "#0432ff"),
    name = "Obesity\nPrevalence (%)\n",
    na.value = "grey95",
  ...
  labs(
    title = "Obesity Prevalence in New York City",
    subtitle = "by Census Tract",
    caption = "Data source: CDC PLACES 2025"
  )
```
Here, too, we notice elevated values of obesity in the Bronx along with southeastern Queens and eastern Brooklyn.

```{r}
#| code-summary: "CODE: NYC vs. USA"
#| message: false
#| warning: false
# National stats
national_avg <- tracts_health %>%
  st_drop_geometry() %>%
  summarise(
    national_diabetes = mean(diabetes, na.rm = TRUE),
    national_obesity = mean(obesity, na.rm = TRUE)
  )
...
nyc_avg <- nyc_data %>%
  st_drop_geometry() %>%
  summarise(
    nyc_diabetes = mean(diabetes, na.rm = TRUE),
    nyc_obesity = mean(obesity, na.rm = TRUE)
  )
```
The average rate of of diabetes in NYC (by census tract) is **`r round(nyc_avg$nyc_diabetes, 2)`%** vs. **`r round(national_avg$national_diabetes, 2)`%** for the whole country. The average rate of of obesity in NYC is **`r round(nyc_avg$nyc_obesity, 2)`%** vs. **`r round(national_avg$national_obesity, 2)`%**. We can conclude that at least in these metrics, New York is a healthier American place than average.

Can we say something more about the urban/rural divide related to these conditions?

```{r}
#| code-summary: "CODE: Urban vs. rural"
#| message: false
#| warning: false
# Define urban/rural based on population density thresholds
tracts_health <- tracts_health %>%
  mutate(
    urban_rural = case_when(
      pop_density >= 3000 ~ "Urban Core",
      pop_density >= 1000 ~ "Urban",
      pop_density >= 500 ~ "Suburban",
      pop_density >= 100 ~ "Rural",
      TRUE ~ "Very Rural"
    ),
    # Make it an ordered factor for proper sorting
    urban_rural = factor(urban_rural, 
                         levels = c("Urban Core", "Urban", "Suburban", "Rural", "Very Rural"))
  )

# Compare rates
urban_rural_summary <- tracts_health %>%
  st_drop_geometry() %>%
  ...
  summarise(
    ...
    total_population = sum(totalpopulation, na.rm = TRUE),
    avg_diabetes = mean(diabetes, na.rm = TRUE),
    median_diabetes = median(diabetes, na.rm = TRUE),
    avg_obesity = mean(obesity, na.rm = TRUE),
    median_obesity = median(obesity, na.rm = TRUE),
    .groups = "drop"
  )

urban_rural_summary |> format_titles() |>
  datatable(
    options = list(
      searching = FALSE,
      paging = FALSE,
  ...
  formatRound(c('Avg Diabetes', 'Median Diabetes', 'Avg Obesity', 'Median Obesity'), digits = 1) |>
  formatRound(c('N Tracts', 'Total Population'), digits = 0, mark=',')
```
<br>
This table seems to further support the notion that the prevalence of obesity and diabetes increases as population density is reduced. Why would this be the case? One could surmise that greater access to healthcare, more walkable neighborhoods, and younger, more affluent populations could be factors. 

Finally let's use a statistical test to be a bit more certain. Using a population density of 1,000 people per square mile to divide census tracts into urban and rural, let's test a hypothesis.

**H~0~: The rural rate of diabetes - the urban rate of diabetes = 0**

**H~a~: The rural rate of diabetes - the urban rate of diabetes ≠ 0**

```{r}
#| code-summary: "CODE: t-test for diabetes"
#| message: false
#| warning: false
# Two-category classification
tracts_health <- tracts_health %>%
  mutate(
    location_type = ifelse(pop_density >= 1000, "Urban", "Rural")
  )

# T-test for diabetes significance
library(infer)
tracts_health |> t_test(diabetes ~ location_type, order=c("Urban", "Rural")) |>
  format_titles() |>
  datatable(
    options = list(
      searching = FALSE,
  ...
  formatRound(c('Statistic', 'T Df', 'P Value', 'Estimate', 'Lower Ci', 'Upper Ci'), digits = 3) 

```
<br>
As we can see from the p-value of 0, there is a statistically significant difference in diabetes rates between urban and rural areas.

Similarly, let's test another hypothesis for obesity.

**H~0~: The rural rate of obesity - the urban rate of obesity = 0**

**H~a~: The rural rate of obesity - the urban rate of obesity ≠ 0**

```{r}
#| code-summary: "CODE: t-test for obesity"
#| message: false
#| warning: false

# T-test for obesity significance
tracts_health |> t_test(obesity ~ location_type, order=c("Urban", "Rural")) |>
  format_titles() |>
  datatable(
    options = list(
      searching = FALSE,
...
```
<br>
Again, as we can see from the p-value of 0, there is a statistically significant difference in obesity rates between urban and rural areas.
