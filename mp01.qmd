...
The top non-English film to trend globally is **`r top_non_english_film$show_title`** with **`r top_non_english_film$cumulative_weeks_in_top_10`** weeks in the top 10. The longest film is **`r longest_film$show_title`** at **`r round(longest_film$runtime_in_minutes, 2)`** minutes. I'll bet you are wondering which of our programs had the most viewership by category. The results are in.
...
GLOBAL_TOP_10 |>
  ...
  datatable(
   ...
   formatRound(c('Weekly Hours Viewed'), digits = 0) 
```
...
### Press Release 2: Hindi-language programming is taking off
```{r}
...
one_this_week_korea <- COUNTRY_TOP_10 |>
  ...
  pull(show_title)
```
The number one TV show in Korea this week is  **`r one_this_week`**. If we look at the intersection of the top programs in South Korea (that are also popular in the United States), we notice something interesting. About half of the shows are of Korean origin. The remainder are of Japanese and US origin.
