...
### How to use Computationally-Intensive Inference

```{mermaid}
flowchart LR
  A[Your observed data] --> B(simulated dataset)
  A[Your observed data] --> C(simulated dataset)
  A[Your observed data] --> D(simulated dataset)
  A[Your observed data] --> E((etc.))
  B --> F[simulated results histogram marked with actual observation]
  C --> F[simulated results histogram marked with actual observation]
  D --> F[simulated results histogram marked with actual observation]
  E --> F[simulated results histogram marked with actual observation]
```

This is a high-level process flow to illustrate Computationally-Intensive Inference. An example of the output histogram from earlier on the page is re-displayed here below.

```{r}
#| code-summary: "CODE: Computationally-intensive inference"
#| message: false
#| warning: false
null_dist_1_sample |>
  visualize() +
